 ...
 * Adds console logs for each detected block.
 *
...
export function buildLabelBlocks(spans) {
	...
	while (i < items.length) {
		...
		const block = { labelText, left, topStart, topEnd, page };
		console.log(`[special] Label block: "${block.labelText}" page=${page} left=${left} topStart=${topStart} topEnd=${topEnd}`);
		blocks.push(block);
		i = j;
	}
